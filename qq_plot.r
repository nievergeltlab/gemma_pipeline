...
library(data.table)
data <- fread(paste('zcat ', pvals,sep=''),data.table=F)
...
plot(c(0,max(QQ)), c(0,max(UNADJ)+ 1), xlab='Expected -log10(p)', ylab='Observed -log10(p)', col='blue', cex=1.3, cex.axis=1.2, cex.lab=1.5,pch=20)
